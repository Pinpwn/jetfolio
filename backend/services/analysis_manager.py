...
def get_llm_service(session: Session) -> LLMService:
    ...
    if provider == "perplexity":
        config_key = session.get(Config, "perplexity_api_key")
        api_key = get_secure_config().get_value(config_key) if config_key else None
    elif provider == "groq":
        config_key = session.get(Config, "groq_api_key")
        api_key = get_secure_config().get_value(config_key) if config_key else None
...
class AnalysisManager:
    ...
    def process_pending_articles_background(self):
        """
        Trigger processing in a separate thread to avoid blocking API.
        Uses asyncio.run to execute the async loop.
        """
        thread = threading.Thread(target=lambda: asyncio.run(self.process_pending_articles()))
        thread.daemon = True
        thread.start()
        ...
        while True:
            try:
                with Session(engine) as session:
                    ...
                    pending_articles = session.exec(query).all()
